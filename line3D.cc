...
{
    //------------------------------------------------------------------------------
    Line3D::Line3D(const std::string& output_folder,const std::string& inputFolder, const std::string& inputFolderMatches,
                    const bool load_segments,
                   const int max_img_width,
                   const unsigned int max_line_segments,
                   const bool neighbors_by_worldpoints,
                   const bool use_GPU,
                   const bool matchepip) :
        data_folder_(output_folder+"/L3D++_data/"),data_input_folder_(inputFolder),data_matches_folder_(inputFolderMatches),
                          ...
                          const std::vector<cv::Vec4f>& line_segments)
    {
        ...
        //views_avg_depths_.push_back(L3D_EPS);
        //std::cout << " [0]" << std::endl;
    ...
    L3DPP::DataArray<float4>* Line3D::detectLineSegments(const unsigned int camID, const cv::Mat& image)
    {
        ...
        std::vector<Eigen::Vector4f> info_write;
        ...
        if(sorted.size() > 0)
        {
            ...
            //writeFile_line(camID, info_write);
    ...
    void Line3D::writeFile_line(const unsigned int id_cam, std::vector<Eigen::Vector4f> segments2d)
	{
	  std::stringstream str;
	  str << data_folder_ << "segments_L3D++_";

	  boost::filesystem::path dir(str.str());

	  std::ofstream myfile(dir.string()+"2d_lines_camid"+std::to_string(id_cam)+".txt");
	  //std::cout << dir.string()+"2d_lines_camid"+std::to_string(id_cam)+".txt"<< std::endl;

  	  std::vector<Eigen::Vector4f>::const_iterator it0 = segments2d.begin();
          for(; it0!=segments2d.end(); ++it0)
          {
              Eigen::Vector4f coords=*it0;
              std::string info = std::to_string(coords(0))+" "+std::to_string(coords(1))+" "+std::to_string(coords(2))+" "+std::to_string(coords(3));
	   ...
	   myfile.close();
	   //else std::cout << "Unable to open file lines last"<< std::endl;
                             ...
                             const int kNN, const float const_regularization_depth)
    {
        ...
        //med_scene_depth_ = views_avg_depths_[views_avg_depths_.size()/2];
         //std::cout <<src << " " << matrix_med[i][1]<< std::endl;
...
/*
    float Line3D::findMedian(vector<float> a,int n)
    {
...
*/
    std::vector<std::vector<float> > Line3D::ComputeWorlpointDepth()
    {
        std::vector<Eigen::Vector3d> cams_worldpoint_3(visual_neighbors_.size());

        std::map<unsigned int,std::vector<std::list<L3DPP::Match> > >::iterator dbg_it = matches_.begin();
        for(; dbg_it!=matches_.end(); ++dbg_it)
        {
                   //std::cout <<"id1 "<< dbg_it->first<< std::endl;

	    	    L3DPP::View* v = views_[dbg_it->first];
		    //std::cout <<"size second "<< dbg_it->second.size()<< std::endl;
		    for(size_t ii=0; ii<dbg_it->second.size(); ++ii)
		    {
		    	//std::cout <<"ii "<< ii<< std::endl;
		        std::list<L3DPP::Match>::iterator dbg_it2 = dbg_it->second.at(ii).begin();
		        //std::cout <<"size second sec"<< dbg_it->second.at(ii).size()<< std::endl;
		        //std::cout <<"size id2 "<< dbg_it->second.at(ii).size()<< std::endl;
		        for(; dbg_it2!=dbg_it->second.at(ii).end(); ++dbg_it2)
		        {
		            L3DPP::Match m = *dbg_it2;
		            L3DPP::Segment3D seg3D = v->unprojectSegment(m.src_segID_,m.depth_p1_,m.depth_p2_);
		            //std::cout <<"cam_id "<< dbg_it->first<< std::endl
		            cams_worldpoint_3.push_back((seg3D.P1()+seg3D.P2())/2);
		            //std::cout << "P1+P2  "<<(seg3D.P1()+seg3D.P2())/2<< std::endl;

		            //std::cout << "C"<< std::endl;
		            //std::cout << std::to_string(C(0))+" "+std::to_string(C(1))+" "+std::to_string(C(2)) << std::endl;
		            //std::cout << "P1"<< std::endl;
		            //std::cout << std::to_string(seg3D.P1()(0))+" "+std::to_string(seg3D.P1()(1))+" "+std::to_string(seg3D.P1()(2)) << std::endl;
		            //std::cout << "P2"<< std::endl;
		            //std::cout << std::to_string(seg3D.P2()(0))+" "+std::to_string(seg3D.P2()(1))+" "+std::to_string(seg3D.P2()(2)) << std::endl;
		            //(seg3D.P1()-C).norm()
		        }
		    }
            ...
            std::cout <<"cams_worldpoint_3.size() "<< cams_worldpoint_3.size()<< std::endl;
            std::vector<std::vector<float> > cams_worldpointDepths(visual_neighbors_.size()+1);
            std::cout <<"visual_neighbors_.size() "<< visual_neighbors_.size()<< std::endl;

            std::map<unsigned int,L3DPP::View*>::const_iterator it22 = views_.begin();

            for(; it22!=views_.end(); ++it22)
            {
                std::cout <<"it22->first "<< it22->first<< std::endl;
	    	    for(int jj=0; jj<cams_worldpoint_3.size(); ++jj){


	    	    	cams_worldpointDepths[it22->first].push_back((it22->second->C()-cams_worldpoint_3[jj]).norm());
	         }
            }
            std::cout <<"end depth"<<  std::endl;
            return cams_worldpointDepths;
    }

    void Line3D::computeMedianDepth()
    {
    std::map<unsigned int,std::set<unsigned int> >::const_iterator it = visual_neighbors_.begin();
    std::cout << "1!" << std::endl;
        std::vector<std::vector<float> > cams_worldpointDepths(visual_neighbors_.size()+1);
        //std::cout << cams_worldpointDepths.size() << std::endl;
        ...
        for(; it!=visual_neighbors_.end(); ++it)
        {
            ...
            for(int i=0; i<matches_[src].size(); ++i)
            {
                ...
                //std::cout << "--" << std::endl;
                std::list<L3DPP::Match>::const_iterator it2 = matches.begin();
                //std::cout << "---" << std::endl;
                for(; it2!=matches.end(); ++it2)
                {
                    //std::cout << (*it2).overlap_score_ << std::endl;
                    //if ((*it2).overlap_score_>0.1){
                    cams_worldpointDepths[src].push_back((*it2).depth_p1_);
                    cams_worldpointDepths[src].push_back((*it2).depth_p2_);
                    //}
                    //std::cout << "--" << std::endl;
                    //std::cout << "--" << std::endl;
                }
        ...
        for(; it!=visual_neighbors_.end(); ++it)
        {
            ...
            if (cams_worldpointDepths[src].size()>0){
                std::sort(cams_worldpointDepths[src].begin(),cams_worldpointDepths[src].end());
                med_depth = cams_worldpointDepths[src][(int)cams_worldpointDepths[src].size()/2];
            }
            else med_depth = L3D_EPS;
            //std::cout << "med_depth " <<med_depth<< std::endl;
            views_avg_depths_.push_back(fmax(med_depth,L3D_EPS));


        }
        std::cout << "3!" << std::endl;
        std::sort(views_avg_depths_.begin(),views_avg_depths_.end());
        med_scene_depth_ = views_avg_depths_[(int)views_avg_depths_.size()/2];


        std::cout  << "Median depth min: " << views_avg_depths_[0] << std::endl;
        std::cout  << "Median depth 25%: " << views_avg_depths_[views_avg_depths_.size()/4] << std::endl;
        std::cout  << "Median depth 50%: " << views_avg_depths_[views_avg_depths_.size()/2] << std::endl;
        std::cout  << "Median depth 75%: " << views_avg_depths_[views_avg_depths_.size()*3/4-1] << std::endl;
        std::cout  << "Median depth max: " << views_avg_depths_[views_avg_depths_.size()-1] << std::endl;
        ...
        for(; it!=visual_neighbors_.end(); ++it)
        {
            ...
            if(!fixed3Dregularizer_){
                views_[src]->update_k(sigma_p_,med_scene_depth_);
    ...
    void Line3D::computeMatches()
    {
        ...
        std::ofstream myfile2(data_input_folder_+"pair_arkit.txt");
        ...
        for(; it!=visual_neighbors_.end(); ++it)
        {
            ...
            for(; n_it!=it->second.end(); ++n_it)
            {
                ...
                if(matched_[it->first].find(*n_it) == matched_[it->first].end())
                {
                    ...
                    // matching
                    if (use_match_epip==true){
                    matchingCPU_epipolar(it->first,*n_it,F);
                    }
                    else
                    {
            ...
            int src = it->first;
              ...
              std::stringstream str;
              str << data_input_folder_ << "matching_epipolar_arkit/";
              boost::filesystem::path dir(str.str());
              boost::filesystem::create_directory(dir);

              std::ofstream myfile(dir.string()+"matches_cam_"+std::to_string(src)+".txt");
              //std::cout << dir.string()+"2d_lines_camid"+std::to_string(id_cam)+".txt"<< std::endl;
              ...
              for(size_t i=0; i<matches_[src].size(); ++i)
            {
                ...
                {
                    info = std::to_string((*it).tgt_camID_)+" "+std::to_string((*it).src_segID_)+" "+std::to_string((*it).tgt_segID_)+" ";
                    if (info != "") myfile << info+"\n";
	 ...
	 //std::ofstream myfile(data_input_folder_+"all_matches.txt");
     ...
     //           {
     //               L3DPP::Match m = *dbg_it2;
     //               std::string info = std::to_string(m.src_camID_)+" "+std::to_string(m.src_segID_)+" "+std::to_string(m.tgt_camID_)+" "+std::to_string(m.tgt_segID_);
                    //std::cout << info<< std::endl;
      //              myfile << info+"\n";
    ...
    Eigen::Matrix3d Line3D::getFundamentalMatrix(L3DPP::View* src, L3DPP::View* tgt)
    {
        ...
        //std::cout <<"Idx: "<<src->id()<< std::endl;
        //std::cout <<t1(0)<<" "<<t1(1)<<" "<<t1(2)<< std::endl;
        //std::cout <<"R: "<<R1(0,0)<<" "<<R1(0,1)<<" "<<R1(0,2)<<" "<<R1(1,0)<<" "<<R1(1,1)<<" "<<R1(1,2)<<" "<<R1(2,0)<<" "<<R1(2,1)<<" "<<R1(2,2)<< std::endl;
        //std::cout <<"K: "<<K1(0,0)<<" "<<K1(0,1)<<" "<<K1(0,2)<<" "<<K1(1,0)<<" "<<K1(1,1)<<" "<<K1(1,2)<<" "<<K1(2,0)<<" "<<K1(2,1)<<" "<<K1(2,2)<< std::endl;
        ...
        //std::cout<< std::endl;
        //std::cout <<"K1 "<<K1(0,0)<<" "<<K1(0,1)<<" "<<K1(0,2)<<" "<<K1(1,0)<<" "<<K1(1,1)<<" "<<K1(1,2)<<" "<<K1(2,0)<<" "<<K1(2,1)<<" "<<K1(2,2)<< std::endl;
        //std::cout <<"K2 "<<K2(0,0)<<" "<<K2(0,1)<<" "<<K2(0,2)<<" "<<K2(1,0)<<" "<<K2(1,1)<<" "<<K2(1,2)<<" "<<K2(2,0)<<" "<<K2(2,1)<<" "<<K2(2,2)<< std::endl;
        ...
        //std::cout <<"T "<<T(0,0)<<" "<<T(0,1)<<" "<<T(0,2)<<" "<<T(1,0)<<" "<<T(1,1)<<" "<<T(1,2)<<" "<<T(2,0)<<" "<<T(2,1)<<" "<<T(2,2)<< std::endl;

        Eigen::Matrix3d E = T * R;

        //std::cout <<"E "<<E(0,0)<<" "<<E(0,1)<<" "<<E(0,2)<<" "<<E(1,0)<<" "<<E(1,1)<<" "<<E(1,2)<<" "<<E(2,0)<<" "<<E(2,1)<<" "<<E(2,2)<< std::endl;

        Eigen::Matrix3d F = K2.transpose().inverse() * E * K1.inverse();

        //std::cout <<"F "<<F(0,0)<<" "<<F(0,1)<<" "<<F(0,2)<<" "<<F(1,0)<<" "<<F(1,1)<<" "<<F(1,2)<<" "<<F(2,0)<<" "<<F(2,1)<<" "<<F(2,2)<< std::endl;

        fundamentals_[src->id()][tgt->id()] = F;

        return F;
    }
    void Line3D::matchingCPU_epipolar(const unsigned int src, const unsigned int tgt,
                             const Eigen::Matrix3d& F)
    {
        ...
        for(int r=0; r<lines_src->width(); ++r)
        {
            ...
            for(size_t c=0; c<lines_tgt->width(); ++c)
            {
                ...
                //std::cout <<fabs(p1_proj.z())<< std::endl;
    ...
    void Line3D::matchingCPU(const unsigned int src, const unsigned int tgt,
                             const Eigen::Matrix3d& F)
    {
    	//std::cout <<std::to_string(src)<< std::endl;
    	//std::cout <<std::to_string(tgt)<< std::endl;
        ...
        std::string sf3=data_input_folder_+data_matches_folder_+"/matches_cam_"+std::to_string(src)+".txt";
        //std::cout <<sf3<< std::endl;
        std::ifstream file3(sf3);
	std::string str3;
	std::vector<std::vector<int>> matrix_match;
	int ii =0;
	while (std::getline(file3, str3))
	{
		float value;
		std::stringstream ss(str3);
		matrix_match.push_back(std::vector<int>());
		while (ss >> value)
		{
		    matrix_match[ii].push_back(int(value));
	...
	//std::cout <<std::to_string(lines_src->width())<< std::endl;
	for(int r=0; r<matrix_match.size(); ++r)
        {
        	if ((int)matrix_match[r][0]==tgt)
        	{
        	//std::cout <<std::to_string(matrix_match[r].size())<< std::endl;
        	for(int v=1; v<matrix_match[r].size(); ++v)
        	{
        		int new_matches = 0;
        		if (matrix_match[r][v]!=-1)
        		{


			    // source line
			    Eigen::Vector3d p1(lines_src->dataCPU(v-1,0)[0].x,
				               lines_src->dataCPU(v-1,0)[0].y,1.0);
			    Eigen::Vector3d p2(lines_src->dataCPU(v-1,0)[0].z,
				               lines_src->dataCPU(v-1,0)[0].w,1.0);
				...
				if(fabs(p1_proj.z()) > L3D_EPS && fabs(p2_proj.z()) > L3D_EPS)
				{
				    ...
				    //std::cout <<epipolar_overlap_<< std::endl;
			...
			if(kNN_ > 0)
			    {
                    while(new_matches < kNN_ && !scored_matches.empty())
                    {
                        matches_[src][v-1].push_back(scored_matches.top());
                        scored_matches.pop();
                        ++new_matches;
                    }
			    }
        ...
        num_matches_[src] += num_matches;
        //std::cout <<std::to_string(num_matches_[src])<< std::endl;
    }
	...
	*/
            ...
            for(; it!=matches_[src][i].end(); ++it)
            {
                ...
                for(; it2!=matches_[src][i].end(); ++it2)
                {
                    ...
                    if(M.tgt_camID_ != M2.tgt_camID_)
                    {
                        ...
                        std::cout <<data_input_folder_+"info_"+std::to_string(M.src_camID_)+"_"+std::to_string(M.tgt_camID_)+"_"
                          +std::to_string(M2.src_camID_)+"_"+std::to_string(M2.tgt_camID_)+".txt"<<std::endl;

                          std::ofstream myfile(data_input_folder_+"info_"+std::to_string(M.src_camID_)+"_"+std::to_string(M.tgt_camID_)+"_"
                          +std::to_string(M2.src_camID_)+"_"+std::to_string(M2.tgt_camID_)+".txt");

                          L3DPP::Segment3D M3D2 = unprojectMatch(M2,true);

                          Eigen::Vector3d M3D_P1 = M3D.P1();
                          Eigen::Vector3d M3D_P2 = M3D.P2();

                          Eigen::Vector3d M3D2_P1 = M3D2.P1();
                          Eigen::Vector3d M3D2_P2 = M3D2.P2();

                          float angle = angleBetweenSeg3D(M3D,M3D2,true);
                            float sim_a = expf(-angle*angle/two_sigA_sqr_);

                            // positional similarity
                            float d1 = M.depth_p1_-M2.depth_p1_;
                            float d2 = M.depth_p2_-M2.depth_p2_;

                            float sim_p = fmin(expf(-d1*d1/reg1),expf(-d2*d2/reg2));



                          std::string info = std::to_string(M3D_P1(0))+" "+std::to_string(M3D_P1(1))+" "+std::to_string(M3D_P1(2))+" "+
                          std::to_string(M3D_P2(0))+" "+std::to_string(M3D_P2(1))+" "+std::to_string(M3D_P2(2))+" "+
                          std::to_string(M3D2_P1(0))+" "+std::to_string(M3D2_P1(1))+" "+std::to_string(M3D2_P1(2))+" "+
                          std::to_string(M3D2_P2(0))+" "+std::to_string(M3D2_P2(1))+" "+std::to_string(M3D2_P2(2))+" "+
                          std::to_string(sim)+" "+std::to_string(sim_a)+" "+std::to_string(sim_p)+" ";
                          myfile << info+"\n";
                          myfile.close();
                ...
                //std::cout << "score3D"<<std::endl;
                //std::cout << std::to_string(score3D)<<std::endl;
    ...
    void Line3D::clusterSegments()
    {
        ...
        std::cout <<"init graph clustering"<< std::endl;
        // graph clustering
        L3DPP::CLUniverse* u = L3DPP::performClustering(A_,global2local_.size(),3.0f);
        std::cout <<"graph clustering"<< std::endl;
        ...
        std::cout <<cluster2segments.size()<< std::endl;
        ...
        std::cout << prefix_ << "clusters: " << "total=" << cluster2segments.size() << ", ";
